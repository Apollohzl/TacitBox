...
export async function GET(request: NextRequest) {
  try {
    ...
    const [participations] = await connection.execute(
      'SELECT activity_id, participant_user_id, answers,correct_count, has_rewarded,participation_time, participant_user_type FROM quiz_participations WHERE activity_id = ?',
